...
#[tokio::test]
async fn stale_local_curated_cache_uses_synced_first_party_plugin() -> Result<()> {
    let fixture = run_lifecycle_fixture_with_stale_local_cache("printf 'ok\n'\n").await?;
    assert_eq!(fixture.events.len(), 2);
    assert_eq!(
        event_with_status(&fixture.events, "started")?["event_params"]["plugin_id"],
        PLUGIN_CONFIG_NAME
    );
    Ok(())
}
...
async fn run_lifecycle_fixture_with_stale_local_cache(script: &str) -> Result<LifecycleFixture> {
    run_lifecycle_fixture_inner(
        script,
        &[],
        /*interrupt*/ false,
        FIRST_PARTY_PLUGIN,
        /*stale_local_cache*/ true,
    )
    .await
}
...
async fn run_lifecycle_fixture(
    ...
    run_lifecycle_fixture_inner(
        ...
        interrupt,
        identity,
        /*stale_local_cache*/ false,
    )
    .await
}

async fn run_lifecycle_fixture_inner(
    script: &str,
    script_args: &[&str],
    interrupt: bool,
    identity: PluginIdentity,
    stale_local_cache: bool,
) -> Result<LifecycleFixture> {
