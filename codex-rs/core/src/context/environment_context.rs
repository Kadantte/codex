...
pub(crate) struct EnvironmentContextEnvironment {
    pub(crate) id: String,
    pub(crate) cwd: NativePathString,
...
impl EnvironmentContextEnvironment {
    fn legacy(cwd: AbsolutePathBuf, shell: String) -> Self {
        Self {
            id: String::new(),
            cwd: NativePathString::new(cwd.to_string_lossy()),
            shell,
        }
    }
    ...
    fn from_turn_context_environment(environment: &TurnContextEnvironment) -> Option<Self> {
        Some(Self {
            id: environment.environment_id.clone(),
            cwd: NativePathString::from_path_uri(&environment.cwd, environment.path_convention)
                .ok()?,
