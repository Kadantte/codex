...
fn test_native_path(unix_path: &str) -> NativePathString {
    NativePathString::new(test_path_buf(unix_path).display().to_string())
}
...
fn serialize_workspace_write_environment_context() {
    ...
    let context = EnvironmentContext::new(
        vec![EnvironmentContextEnvironment {
            id: "local".to_string(),
            cwd: NativePathString::new(cwd.display().to_string()),
    ...
    let context = EnvironmentContext::new(
        vec![
            EnvironmentContextEnvironment {
                id: "local".to_string(),
                cwd: NativePathString::new(local_cwd.display().to_string()),
                shell: "bash".to_string(),
            },
            EnvironmentContextEnvironment {
                id: "remote".to_string(),
                cwd: NativePathString::new(remote_cwd.display().to_string()),
                shell: "bash".to_string(),
            },
        ],
        Some("2026-02-26".to_string()),
