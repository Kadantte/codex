...
/// Tracks one resolved plugin-script invocation through the executor process.
///
/// Resolution happens before process launch, but the first event is not emitted
/// until the executor successfully spawns its process. For classic shell this
/// observes the shell or sandbox wrapper rather than a nested interpreter child.
/// Terminal calls are idempotent because unified exec can observe the same
/// process through several paths.
...
fn command_tokens(command: &str, shell_type: ShellType) -> Option<Vec<String>> {
    match shell_type {
        ...
        ShellType::PowerShell | ShellType::Cmd => split_windows_command(command),
    }
}
