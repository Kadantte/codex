...
fn script_token(command: &str, shell_type: ShellType) -> Option<String> {
    ...
    if let Some(runner_script) = runner_script {
        return (!expands_shell_paths || !has_shell_path_expansion(&runner_script))
            .then_some(runner_script);
    }
    if matches!(
        basename,
    ...
    path_qualified_program
        .then(|| program.clone())
        .filter(|program| !expands_shell_paths || !has_shell_path_expansion(program))
}
