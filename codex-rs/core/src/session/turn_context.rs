...
impl TurnEnvironment {
    ...
    pub(crate) fn new_with_uri(
        ...
        NativePathString::from_path_uri(&cwd, path_convention).map_err(|err| {
    ...
    pub(crate) fn native_cwd(&self) -> NativePathString {
        match NativePathString::from_path_uri(&self.cwd, self.path_convention) {
            Ok(cwd) => cwd,
