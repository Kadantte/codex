...
async fn native_environment_selections_use_selected_environment_metadata() {
    ...
    let selections = manager
        .resolve_native_environment_selections([(
            "local".to_string(),
            NativePathString::new(config.cwd.display().to_string()),
        )])
        .await
...
async fn native_environment_selections_reject_duplicate_ids() {
    ...
    let native_cwd = NativePathString::new(config.cwd.display().to_string());
    let error = manager
        .resolve_native_environment_selections([
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "invalid cwd for environment `local`: native path `relative` is not valid absolute {} path syntax",
