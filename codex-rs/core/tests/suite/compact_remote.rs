...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn remote_mid_turn_compact_v1_sends_turn_state_across_websocket_and_http() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    // Phase 2: the HTTP compact request receives the state established by WebSocket sampling.
    ...
    // Phase 3: WebSocket continuation keeps replaying that first value.
    let requests = server
        .connections()
        .into_iter()
        .flatten()
